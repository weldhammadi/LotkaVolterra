...
for alpha, beta, gamma, delta in product(alpha_values, beta_values, gamma_values, delta_values):
    ...
    for _ in range(100_000):
        ...
        if _ < len(real_data):
            mse_lapin = (lapin[-1] - real_data["lapin"].iloc[_]) ** 2
            mse_renard = (renard[-1] - real_data["renard"].iloc[_]) ** 2
            mse += mse_lapin + mse_renard
    mse = mse / (2 * len(real_data))
    print(f'Best parameters: alpha={alpha}, beta={beta}, gamma={gamma}')
...
print(f'Best parameters: alpha={alpha}, beta={beta}, gamma={gamma}, delta={delta}')
...
for _ in range(100_000):
    ...
    if _ < len(real_data):
        mse_lapin = (model_lapin[-1] - real_data["lapin"].iloc[_]) ** 2
        mse_renard = (model_renard[-1] - real_data["renard"].iloc[_]) ** 2
        mse += mse_lapin + mse_renard
